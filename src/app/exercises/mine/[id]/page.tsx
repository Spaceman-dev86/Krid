...
type Props = {
  ...
  searchParams?:
    | Promise<{ error?: string; saved?: string; transferred?: string }>
    | { error?: string; saved?: string; transferred?: string }
}

export default async function EditMineExercisePage({ params, searchParams }: Props) {
  ...
  if (!exercise || exercise.coach_id !== user.id) {
    redirect('/exercises/mine?error=' + encodeURIComponent('Exercice introuvable'))
  }

  return (
    <CoachAppShell appName={shell.branding?.app_name} trialLabel={shell.trialLabel} title="Éditer" savUnread={shell.savUnread}>
      <div className="mx-auto grid max-w-3xl gap-5">
        <div className="flex items-center justify-between gap-2">
          <PageTitle className="text-2xl">{exercise.name}</PageTitle>
          <Link href="/exercises/mine" className="text-sm font-semibold text-[color:var(--brand)] underline">
            ← Ma biblio
          </Link>
        </div>
        ...
        {q.saved || q.transferred ? (
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
            {q.transferred ? 'Copie Trainly ajoutée à ta biblio.' : 'Enregistré.'}
          </div>
        ) : null}
        <ExerciseForm mode="mine" exercise={exercise} />
      </div>
    </CoachAppShell>
  )
}
